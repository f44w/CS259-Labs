...
public class Tests {
	...
	static int NumberOfFeatures =  3; // C
		...
		for (int x = 0; x < NumberOfFeatures; x++) { // Update the score s according to the Naive Bayes formula for the odds: log O(C|x1,x2,x3,...) = log O(C) + log s1 + log s2 + log s3,
			// where s1 = P(x1|C)/P(x1|~C), s2 = P(x2|C)/P(x2|~C), s3 = P(x3|C)/P(x3|~C) are "feature strengths".    
	   		...
	   		if(X[x] > 0) { // To simplify the model and to apply it later to MNIST, we only consider feature presence, not absences   
	   			...
	   			double p_feature_cond_neg =  FeatureCountsNeg[x]/neg_count   ; // C	 	   			
	...
	public static void main(String[] args) throws Exception  {  
		...
		Assert(FeatureCountsPos[2] ==  2); // C Finish those lines, but don't remove them!  
		...
		System.out.print("P(fever|flu)= "); System.out.println(FeatureCountsPos[1]/pos_count);  
		System.out.print("P(fever|~flu)= "); System.out.println(FeatureCountsNeg[1]/neg_count); 
		System.out.print("P(fever|sneezing)= "); System.out.println(FeatureCountsPos[2]/pos_count);  
		System.out.print("P(fever|~sneezing)= "); System.out.println(FeatureCountsNeg[2]/neg_count); 
		...
		Assert(Math.abs(M.estimate(new double [] {1,0,0})  - .97) < 0.01); // Replace P with the theoretical estimate here for comparison  
		System.out.print("P(flu|fever)= "); System.out.println(M.estimate(new double [] {0,1,0})); 
		Assert(Math.abs(M.estimate(new double [] {0,1,0}) - .66) < 0.01); // Replace P with the theoretical estimate here for comparison
		System.out.print("P(flu|sneez)= "); System.out.println(M.estimate(new double [] {0,0,1})); 
		Assert(Math.abs(M.estimate(new double [] {0,0,1}) - .66) < 0.01); // Replace P with the theoretical estimate here for comparison
		System.out.print("P(flu|fever, sneez)= "); System.out.println(M.estimate(new double [] {0,1,1}));
		Assert(Math.abs(M.estimate(new double [] {0,1,1}) - .80) < 0.01);  // Replace P with the theoretical estimate here for comparison
		System.out.print("P(flu|cough, fever, sneez)= "); System.out.println(M.estimate(new double [] {1,1,1}));
		Assert(Math.abs(M.estimate(new double [] {1,1,1}) - .99) < 0.01); // Replace P with the theoretical estimate here for comparison		
