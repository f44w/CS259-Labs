...
public class Tests2Sol {
	...
	static  int  image_size =  28 * 28;  // This should be the total number of pixels // C  
	...
	static Image[] Read(String fileName) throws Exception { 
	  ...
	  int number_of_images =   number_of_lines / 28; // Replace 'X' with the specific number // C
	  br.close();
		  ...
		  for(int row = 0; row < 28; row ++) { // Read all rows:  
			...
			String[] line_parts = st.split("[\t ]+"); 
		 	...
		 	for(int column  = 0; column < 28; column ++) { // Read all columns:
		 			...
		 			if (value > 200) // C Binarizing pixel values by a specified threshold works better: 
				 		images[i].set(row, column, 1);
		 			else
				 		images[i].set(row, column, 0);
			 		// images[i].set(row, column, value); // This gives 74 on train, 71 on test
		 	}
		  }
	  }
	  return images;
