...
public class Tests2 {
    ...
    static  int pos_count = 3, neg_count = 3; // Counts of positive (flu) and negative (~flu) datapoints.

    static int NumberOfFeatures = 3 ;
        ...
        double estimate(double [] X){  // Returns the probability of the datapoint with the features X to belong to the positive class C, here C = having a flu
            ...
            for (int x = 0; x < NumberOfFeatures; x++) { // Update the score s according to the Naive Bayes formula for the odds: log O(C|x1,x2,x3,...) = log O(C) + log s1 + log s2 + log s3,
                ...
                if(X[x] > 0) { // To simplify the model and to apply it later to MNIST, we only consider feature presence, not absences
                    ...
                    double p_feature_cond_neg = FeatureCountsNeg[x]/neg_count; // P(x|~C) = #{x AND ~pos} / #{~pos}
    ...
    public static void main(String[] args) throws Exception  {
        ...
        Assert(FeatureCountsPos[2] == 2); // Finish those lines, but don't remove them!
        Assert(FeatureCountsNeg[0] == 0);
        Assert(FeatureCountsNeg[1] == 1);
        Assert(FeatureCountsNeg[2] == 1);
        ...
        System.out.print("P(fever|flu)= "); System.out.println(FeatureCountsPos[1]/pos_count );  // Finish those lines, but don't remove them!
        System.out.print("P(fever|~flu)= "); System.out.println(FeatureCountsNeg[1]/neg_count );
        System.out.print("P(sneezing|flu)= "); System.out.println(FeatureCountsPos[2]/pos_count );
        System.out.print("P(sneezing|~flu)= "); System.out.println(FeatureCountsNeg[2]/neg_count );
        ...
        Assert(Math.abs(M.estimate(new double [] {1,0,0})  - 0.94) < 0.01); // Replace P with the theoretical estimate here for comparison
        System.out.print("P(flu|fever)= "); System.out.println(M.estimate(new double [] {0,1,0}));
        Assert(Math.abs(M.estimate(new double [] {0,1,0}) - 0.66) < 0.01); // Replace P with the theoretical estimate here for comparison
        System.out.print("P(flu|sneeze)= "); System.out.println(M.estimate(new double [] {0,0,1}));
        Assert(Math.abs(M.estimate(new double [] {0,0,1}) - 0.66) < 0.01); // Replace P with the theoretical estimate here for comparison
        System.out.print("P(flu|fever, sneeze)= "); System.out.println(M.estimate(new double [] {0,1,1}));
        Assert(Math.abs(M.estimate(new double [] {0,1,1}) - 0.8) < 0.01);  // Replace P with the theoretical estimate here for comparison
        System.out.print("P(flu|cough, fever, sneeze)= "); System.out.println(M.estimate(new double [] {1,1,1}));
        Assert(Math.abs(M.estimate(new double [] {1,1,1}) - 0.985) < 0.01); // Replace P with the theoretical estimate here for comparison
