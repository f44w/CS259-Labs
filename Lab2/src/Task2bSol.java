...
public class Task2bSol {
	...
	public static void main(String[] args) throws Exception  {  
		...
		for(int trial=0;trial<1000000000;trial++) { // C  
			...
			if (sick) {
				sick_total ++; 
				if (random.nextInt(1, 10001) == 1) // 99% accuracy test, so only 1 out of 100 tests makes a mistake.
				...
				healthy_total++; // C
				if (random.nextInt(1, 10001) == 1) // C 
				// if (random.nextInt(1, 101) == 1) // C 
					healthy_positive ++; 
				else
					healthy_negative ++;
		...
		double p_Bayes =  .0001*.9999/(.9999*.0001+.0001*.9999)    ; // C 		
		//double p_Bayes =  .0001*.99/(.99*.0001+.01*.9999)    ; // C 		
